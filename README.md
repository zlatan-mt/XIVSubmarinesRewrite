...
このリポジトリは利用者向けに最適化されており、開発用のドキュメントや計画書は含まれていません。

詳細な開発資料（設計書、計画書、セッション記録など）は、開発用のローカルブランチで管理されています。

開発に参加される場合は、以下の情報をご確認ください：

- **ソースコード**: このリポジトリの `src/` ディレクトリ
- **テスト**: `tests/` ディレクトリの Playwright と .NET テスト
- **開発ツール**: `tools/` ディレクトリの各種ユーティリティ
- **開発資料**: 開発用ローカルブランチ（`dev`）で管理
