...
public sealed class MainWindowRenderer : IViewRenderer
{
    ...
    {
        if (ImGui.BeginTable("##main_toolbar", 2, ImGuiTableFlags.SizingStretchProp | ImGuiTableFlags.PadOuterX))
        {
            ...
            ImGui.SetCursorPosX(cursorX + MathF.Max(0f, width - 120f));
            if (ImGui.SmallButton(this.showDeveloperTools ? "DEV: ON" : "DEV: OFF"))
            {
